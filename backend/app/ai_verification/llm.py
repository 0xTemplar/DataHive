...
class LLMWrapper(Runnable):
    def __init__(
        ...
        This version for Lilypad now validates that the model being used is supported.

        This class provides a unified interface for working with Lilypad LLMs while handling provider-specific implementation details.
        ...
        if self.provider == "lilypad":
            if self.model not in SUPPORTED_MODELS:
                raise ValueError(
                    f"Model '{self.model}' is not supported. Supported models are: {SUPPORTED_MODELS}"
                )
            # Instantiate a chat client with the Lilypad API endpoint.
            self.llm = ChatOpenAI(
